...
type EyeColor = string | InnerOuterEyeColor | undefined;
...
export class QRCode extends React.Component<IProps, {}> {
    ...
    private drawPositioningPattern(
        ...
        // Outer box
        this.drawRoundedSquare(lineWidth, x, y, size, colorOuter ?? '', radiiOuter as any, false, ctx);

        // Inner box
        size = cellSize * 3;
        y += cellSize * 2;
        x += cellSize * 2;
        this.drawRoundedSquare(lineWidth, x, y, size, colorInner ?? '', radiiInner as any, true, ctx);
    };
    ...
    update() {
        ...
        const size = this.props.size ? +this.props.size : 0;
        const quietZone = this.props.quietZone ? +this.props.quietZone : 0;
        ...
        ctx.strokeStyle = fgColor as string;
        if (qrStyle === 'dots') {
    ...
    render() {
        const qrSize = +(this.props.size ?? 50) + (2 * +(this.props.quietZone ?? 0));
        return React.createElement('canvas', {
            id: this.props.id ?? 'cumin-qr-code-canvas',
            height: qrSize,
            width: qrSize,
            style: { height: qrSize + 'px', width: qrSize + 'px', border  : '1px solid #E2E8F0', borderRadius : '6px' },
            ref: this.canvas
        });
    }
}